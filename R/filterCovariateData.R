...
#' @export
filterPlpDataEmergentPatterns <- function(plpData, minimumSupport, patternLength, createSets = c("freqPatsOnly", "mix")){
  ...
  keepCovariates <- covariateData$covariateRef %>% 
    dplyr::filter(analysisId == 999 & (patternLength.y <= patLen | patternLength.x <= patLen)  & (support.x >= minSup | support.y >= minSup)) %>% 
    dplyr::pull(covariateId)
